...
export function Renderer(ctx: CanvasRenderingContext2D | null, width: number, height: number): Render {
  ...
  function updateTerrain(t: number) {
    ...
    iterateGrid2D(grid, (p, x, y, i) => {
      ...
      const roadWinding = Math.sin((y - terrainOffset) / cornerPeriod) * roadTwistyness;
      const road = Math.max(
          roadWidth - 1,
      ...
      const mountains =
          mountainOffset + noise2D(x * noiseScale, (y - terrainOffset) * noiseScale) * mountainHeight * hilliness;
  ...
  function drawGrid2D() {
    ...
    function isInvisible(x: number, y: number) {
      const vertices = [index(x, y), index(x + 1, y), index(x + 1, y + 1), index(x, y + 1)];
      ...
      return magnitudeACrossB < 0 && magnitudeACrossC < 0;
    }
